...
/// Zero-knowledge non-interactive proof of two vectors `a` and `b` and their inner product that
/// scales logarithmically with the length of the vectors
fn main() {
    ...
    // Prover generates random value by hashing all commitments that exist up until this point, to
    // avoid the need to have the verifier send a random value, and thus make the proof
    // non-interactive (ie, using the Fiat-Shamir heuristic)
    let mut hasher = DefaultHasher::new();
    (
        l_r_constant_comm + l_r_linear_comm + t_constant_comm,
        t_linear_comm + t_quadratic_comm,
    )
        .hash(&mut hasher);
    ...
    (
        l_r_constant_comm + l_r_linear_comm + t_constant_comm,
        t_linear_comm + t_quadratic_comm + initial_concatenated_inner_product,
    )
        .hash(&mut hasher);
    let u_1 = Fr::from(hasher.finish());
    ...
    (
        l_r_constant_comm + l_r_linear_comm + t_constant_comm,
        t_linear_comm + t_quadratic_comm + initial_concatenated_inner_product + p,
    )
        .hash(&mut hasher);
    let u_2 = Fr::from(hasher.finish());
