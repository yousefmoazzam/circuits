...
///
/// The values 4 and 11 common to both set of evaluation values which were interpolated represent
/// two copy constraints between two wires in two different sets (`l_2` and `o_1`, and `l_3` and
/// `o_2`).
///
...
fn main() {
    ...
    // - `omega` to `g` (rather than to `omega`)
    // - `omega^2` to `g omega^2` (rather than to `omega^2`)
    ...
    // Accounts for the "permutation" that occurs which maps:
    // - 1 to `omega` (rather than to `g`)
    // - `omega` to `omega^2` (rather than to `g omega`)
